...
.wrap{position:relative;z-index:1;max-width:960px;margin:0 auto;padding:56px 24px 100px}
...
.templates-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;margin-bottom:48px}
...
.tpl-check{position:absolute;top:10px;right:10px;width:26px;height:26px;border-radius:50%;background:var(--blue);display:grid;place-items:center;opacity:0;transform:scale(0.6);transition:all .18s}
.tpl-card.selected .tpl-check{opacity:1;transform:scale(1)}

/* FORM PANNEAU */
.form-panel{background:var(--s1);border:1px solid rgba(59,130,246,.2);border-radius:20px;padding:36px;margin-bottom:20px;box-shadow:0 0 60px rgba(59,130,246,.05);display:none}
...
.field input,.field select,.field textarea{background:var(--s2);border:1px solid rgba(255,255,255,.07);border-radius:11px;padding:12px 14px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:14px;outline:none;transition:border-color .2s,box-shadow .2s;width:100%}
        ...
        <div class="result-panel" id="resultPanel">
            ...
            <div class="gemini-tip">
                ...
                <div class="gemini-tip-text">
                    ...
                    Va sur <a href="https://gemini.google.com" target="_blank">Gemini.google.com</a>, clique sur <b>Gemini 2.0 Flash Experimental</b>, colle le prompt, joins ta photo, et génère. Pour Midjourney : colle dans Discord et ajoute <code style="background:var(--s1);padding:1px 5px;border-radius:4px;font-size:11px">--ar 1:1 --v 6</code>.
                </div>
            </div>
        </div>
    </div>
...
// Labels lisibles pour les variables connues
const VAR_LABELS = {
...
function selectTemplate(card) {
    ...
    const title = card.querySelector('.tpl-titre').textContent;
...
function onFieldInput() {
    // Activer le bouton dès qu'au moins un champ est rempli
