        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center py-4 sm:py-0 sm:h-16 gap-3">
                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    <div>
                        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">{{ $llmConfig->display_name }}</h1>
                        <p class="text-xs text-gray-500">
                            <a href="{{ route('admin.dashboard') }}" class="hover:text-gray-700">Dashboard</a>
                            <span class="mx-1">›</span>
                            <a href="{{ route('admin.llm.index') }}" class="hover:text-gray-700">LLM</a>
                            <span class="mx-1">›</span>
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-2">
                    @if($llmConfig->is_primary)
                        <span class="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-xs font-medium">Provider principal</span>
                    @endif
                    @if($llmConfig->is_active)
                        <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">Actif</span>
                    @else
                        <span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs font-medium">Inactif</span>
                    @endif
                </div>
    ...
    Réinitialiser stats
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
            <div class="p-4 sm:p-6 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500 mt-1">Paramètres du provider IA</p>
            </div>
            ...
            <form action="{{ route('admin.llm.update', $llmConfig->provider) }}" method="POST" class="p-4 sm:p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="display_name" name="display_name" value="{{ $llmConfig->display_name }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500" required>
                        </div>
                        <div>
                        ...
                        <div>
                            <label for="api_url" class="block text-sm font-medium text-gray-700 mb-1">URL API</label>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label for="temperature" class="block text-sm font-medium text-gray-700 mb-1">Température</label>
                            <input type="number" id="temperature" name="temperature" value="{{ $llmConfig->temperature }}"
                                   min="0" max="2" step="0.1"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500" required>
                        </div>
                        <div>
                            <label for="timeout_seconds" class="block text-sm font-medium text-gray-700 mb-1">Timeout (secondes)</label>
                        ...
                        <div class="flex items-center">
                            ...
                            <label for="is_active" class="ml-2 text-sm text-gray-700">Provider actif</label>
                        </div>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="p-4 sm:p-6 space-y-3">
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-gray-600">Total utilisations</span>
                        <span class="font-medium text-gray-900">{{ $llmConfig->usage_count }}</span>
                    </div>
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-gray-600">Succès</span>
                    ...
                    <div class="pt-3 border-t border-gray-100 text-sm text-gray-500">
                        Dernière utilisation : <span class="font-medium text-gray-700">{{ $llmConfig->last_used_at->diffForHumans() }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="p-4 sm:p-6 border-b border-gray-100">
                    <h3 class="text-lg font-semibold text-gray-900">Statut API</h3>
                </div>
                <div class="p-4 sm:p-6 space-y-3">
                    <div class="flex justify-between items-center text-sm">
                    ...
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="font-medium text-gray-900">#{{ $llmConfig->priority_order }}</span>
                    </div>
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-gray-600">Statut</span>
                        <span class="font-medium {{ $llmConfig->is_active ? 'text-green-600' : 'text-gray-500' }}">
                    ...
                    <div class="pt-3 border-t border-gray-100">
                        ...
                        <div class="mt-1 p-2 bg-red-50 rounded text-red-700 text-xs font-mono">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="p-4 sm:p-6 border-b border-gray-100">
                <h3 class="text-lg font-semibold text-gray-900">Test de connexion</h3>
                <p class="text-sm text-gray-500 mt-1">Vérifiez la disponibilité du provider en temps réel</p>
            </div>
            <div class="p-4 sm:p-6">
                <div id="test-result" class="text-sm text-gray-500">
            ...
            if (data.success) {
                resultDiv.innerHTML = `
                    <div class="p-3 bg-green-50 border border-green-100 rounded-lg text-green-700">
                        ...
                        <pre class="mt-2 bg-green-100 rounded p-2 text-xs overflow-auto">${JSON.stringify(data.data, null, 2)}</pre>
                    </div>`;
            } else {
                resultDiv.innerHTML = `
