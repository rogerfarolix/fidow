        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center py-4 sm:py-0 sm:h-16 gap-3">
                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">Générations de positionnement</h1>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
            <div class="bg-white dark:bg-[#1a1a1d] rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-white/5 flex items-center justify-between">
                <div>
                    <div class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $generations->total() }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-300">Total générations</div>
                </div>
            ...
            <div class="bg-white dark:bg-[#1a1a1d] rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-white/5 flex items-center justify-between">
                <div>
                    <div class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">{{ \App\Models\PositioningGeneration::whereNotNull('phrase_retenue')->count() }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-300">Phrases retenues</div>
                </div>
            ...
            <div class="bg-white dark:bg-[#1a1a1d] rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-white/5 flex items-center justify-between">
                <div>
                    <div class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $allMetiers->count() }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-300">Métiers distincts</div>
                </div>
        ...
        <form method="GET" action="{{ route('admin.data') }}" class="mb-6">
            <div class="flex flex-col sm:flex-row gap-3">
                <input type="text" 
                       ...
                       class="flex-1 px-4 py-2 border border-gray-300 dark:border-white/20 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm">
                <select name="metier" 
                        class="px-4 py-2 border border-gray-300 dark:border-white/20 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm">
        ...
        @if($generations->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5">
                @foreach($generations as $g)
                    <div class="bg-white dark:bg-[#1a1a1d] rounded-xl shadow-sm border border-gray-100 dark:border-white/5 p-4 sm:p-5 flex flex-col hover:shadow-md transition-shadow">
                        <!-- Top row: ID + Date + IP -->
                        ...
                        <!-- Métier & Cible -->
                        <div class="mb-3">
                            <div class="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate" title="{{ $g->metier }}">
                                {{ $g->metier }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5" title="{{ $g->cible }}">
                        ...
                        <div class="mb-3 flex-1">
                            <p class="text-sm text-gray-700 dark:text-gray-200 line-clamp-2" title="{{ $g->phrase_1 }}">
                                {{ $g->phrase_1 }}
                            </p>
                        </div>
                        ...
                        <div class="flex items-center justify-between mt-auto pt-3 border-t border-gray-100 dark:border-white/5">
                            <div>
                                @if($g->phrase_retenue)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-800">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-gray-400">
                                        Aucune
                                    </span>
                                @endif
                            </div>
            ...
            <!-- Pagination -->
            <div class="mt-8 bg-white dark:bg-[#1a1a1d] rounded-xl shadow-sm border border-gray-100 dark:border-white/5 px-4 py-3">
                {{ $generations->links() }}
            </div>
        @else
            <!-- Empty state -->
            <div class="bg-white dark:bg-[#1a1a1d] rounded-xl shadow-sm border border-gray-100 dark:border-white/5 p-12 text-center">
                <svg class="w-16 h-16 text-gray-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">Aucun résultat trouvé</h3>
                <p class="text-gray-500 dark:text-gray-400">Ajustez vos filtres ou réessayez plus tard.</p>
            </div>
        @endif
    </main>
</div>
